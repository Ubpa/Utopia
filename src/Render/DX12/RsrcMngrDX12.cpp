...
struct RsrcMngrDX12::Impl {
	...
	unordered_map<size_t, Texture> textureMap;
...
{
	return RegisterTexture2DArray(upload, id, &tex2D, 1);
}
...
D3D12_CPU_DESCRIPTOR_HANDLE RsrcMngrDX12::GetTextureSrvCpuHandle(size_t id, UINT index) const {
	return pImpl->textureMap.find(id)->second.allocationSRV.GetCpuHandle(index);
}
D3D12_GPU_DESCRIPTOR_HANDLE RsrcMngrDX12::GetTextureSrvGpuHandle(size_t id, UINT index) const {
	return pImpl->textureMap.find(id)->second.allocationSRV.GetGpuHandle(index);
}

UDX12::DescriptorHeapAllocation& RsrcMngrDX12::GetTextureRtvs(size_t id) const {
	return pImpl->textureMap.find(id)->second.allocationRTV;
}
