...
static void ImGui_ImplWin32_UpdateMousePos(ImVec2 offset)
{
    ImGui_ImplWin32_Data* bd = ImGui_ImplWin32_GetBackendData();
    ImGuiIO& io = ImGui::GetIO();
    IM_ASSERT(bd->hWnd != 0);
...
void    ImGui_ImplWin32_NewFrame(ImVec2 offset, ImVec2 size)
{
    ...
    ImGui_ImplWin32_UpdateMousePos(offset);
    ...
    if (bd->LastMouseCursor != mouse_cursor)
    {
        ...
        ImGui_ImplWin32_UpdateMouseCursor(ImGui::GetCurrentContext());
    }
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler_Context(ImGuiContext* ctx, bool ignore_mouse, bool ignore_keyboard, HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    ImGuiIO& io = ImGui::GetIO(ctx);
    ImGui_ImplWin32_Data* bd = (ImGui_ImplWin32_Data*)io.BackendPlatformUserData;// ImGui_ImplWin32_GetBackendData();
    ...
    case WM_XBUTTONDOWN: case WM_XBUTTONDBLCLK:
    {
        ...
        if (!ImGui::IsAnyMouseDown(ctx) && ::GetCapture() == NULL)
            ::SetCapture(hwnd);
    ...
    case WM_XBUTTONUP:
    {
        ...
        if (!ImGui::IsAnyMouseDown(ctx) && ::GetCapture() == hwnd)
            ::ReleaseCapture();
        return 0;
    }
    case WM_MOUSEWHEEL:
