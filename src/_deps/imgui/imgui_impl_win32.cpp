...
static std::map<ImGuiContext*, ImGuiContextWin32Data> g_ImGuiContextWin32DataTable;

// Forward Declarations
// call ImGui_ImplWin32_InitPlatformInterface_Shared() after ImGui_ImplWin32_InitPlatformInterface_Context()
...
{
    if (!::QueryPerformanceCounter((LARGE_INTEGER*)&g_ImGuiContextWin32DataTable[ctx].Time))
        return false;
...
void    ImGui_ImplWin32_Shutdown_Shared()
{
    ...
    g_hWnd = (HWND)0;
    assert(g_ImGuiContextWin32DataTable.empty());
}

void    ImGui_ImplWin32_Shutdown_Context(ImGuiContext* ctx)
{
    g_ImGuiContextWin32DataTable.erase(ctx);
}
...
static void ImGui_ImplWin32_UpdateGamepads(ImGuiContext* ctx)
{
    ...
    if (g_ImGuiContextWin32DataTable[ctx].WantUpdateHasGamepad)
    {
        XINPUT_CAPABILITIES caps;
        g_ImGuiContextWin32DataTable[ctx].HasGamepad = (XInputGetCapabilities(0, XINPUT_FLAG_GAMEPAD, &caps) == ERROR_SUCCESS);
        g_ImGuiContextWin32DataTable[ctx].WantUpdateHasGamepad = false;
    }

    XINPUT_STATE xinput_state;
    io.BackendFlags &= ~ImGuiBackendFlags_HasGamepad;
    if (g_ImGuiContextWin32DataTable[ctx].HasGamepad && XInputGetState(0, &xinput_state) == ERROR_SUCCESS)
    {
...
static void ImGui_ImplWin32_UpdateMonitors(ImGuiContext* ctx)
{
    ...
    g_ImGuiContextWin32DataTable[ctx].WantUpdateMonitors = false;
}

void    ImGui_ImplWin32_NewFrame(ImGuiContext* ctx, ImVec2 offset, float width, float height)
{
    ImGui::WrapContextGuard ImGuiContextGuard(ctx);
    ...
    if (g_ImGuiContextWin32DataTable[ctx].WantUpdateMonitors)
        ImGui_ImplWin32_UpdateMonitors(ctx);

    // Setup time step
    INT64 current_time = 0;
    ::QueryPerformanceCounter((LARGE_INTEGER*)&current_time);
    io.DeltaTime = (float)(current_time - g_ImGuiContextWin32DataTable[ctx].Time) / g_TicksPerSecond;
    g_ImGuiContextWin32DataTable[ctx].Time = current_time;
    ...
    if (g_ImGuiContextWin32DataTable[ctx].LastMouseCursor != mouse_cursor)
    {
        g_ImGuiContextWin32DataTable[ctx].LastMouseCursor = mouse_cursor;
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler_Context(ImGuiContext* ctx, bool ignore_mouse, bool ignore_keyboard, HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_DEVICECHANGE:
        if ((UINT)wParam == DBT_DEVNODES_CHANGED)
            g_ImGuiContextWin32DataTable[ctx].WantUpdateHasGamepad = true;
        return 0;
    case WM_DISPLAYCHANGE:
        g_ImGuiContextWin32DataTable[ctx].WantUpdateMonitors = true;
        return 0;
    }
    return 0;
}
...
void ImGui_ImplWin32_EnableDpiAwareness()
{
    ...
    for (auto& [ctx, ImGuiContextWin32DataTable] : g_ImGuiContextWin32DataTable)
    {
        ImGuiContextWin32DataTable.WantUpdateMonitors = true;
    }
...
static void ImGui_ImplWin32_InitPlatformInterface_Context(ImGuiContext* ctx)
{
    ...
    ImGuiPlatformIO& platform_io = ImGui::WrapContextCall(ctx, ImGui::GetPlatformIO);// ImGui::GetPlatformIO();
    ...
    ImGuiViewport* main_viewport = ImGui::WrapContextCall(ctx, ImGui::GetMainViewport);// ImGui::GetMainViewport();
    ImGuiViewportDataWin32* data = IM_NEW(ImGuiViewportDataWin32)();
    data->Hwnd = g_hWnd;
