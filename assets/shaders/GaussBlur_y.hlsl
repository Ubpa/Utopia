...
float4 PS(VertexOut pin) : SV_Target
{
	...
	float step = 1.f / gRenderTargetSize.y;

	[unroll]
	for(uint i=1; i<5; i++) {
		color += gWeight[i] * (gImage.Sample(gSamplerPointWrap, float2(pin.TexC.x, pin.TexC.y + i * step))
			+ gImage.Sample(gSamplerPointWrap, float2(pin.TexC.x, pin.TexC.y + i * step)));
	}
